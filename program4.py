...
#Program that asks the user to input a sentence, and display:
...
def sentence_analysis():
    ...
    # Here,this line splits the input text into a list.
    words = text.split() 
    ...
    print(f"Total words: {total_words}") # Prints the total number of words in the sentence.
    print(f"Longest word: {longest_word} ({len(longest_word)} Letters)") # Prints the longest word along with its length in letters.
    print(f"Uppercase: {text.upper()}") # Prints the sentence in uppercase using the upper() method.
    print(f"Lowercase: {text.lower()}") # Prints the sentence in lowercase using the lower() method.
    print(f"Title Case: {text.title()}") # Prints the sentence in title case using the title() method.    
    print(f"Reversed:{text[::-1]}") # Prints the sentence in reverse order using slicing with a step of -1.

# Checks if you are running the script directly.
if __name__ == "__main__": 
        sentence_analysis()  # Calls the above defined funtion 