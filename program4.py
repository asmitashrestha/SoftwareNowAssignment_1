...
def sentence_analysis(sentence): # Here, Def tells the python that we are defining a function called sentence_analysis that takes one parameter called sentence. 
    text = input ("Enter a sentence: ") # This line prompts the user to enter a sentence and stores it in the variable text.
    if not text.strip(): # This line checks if the input text is empty or contains only whitespace characters.
        print("No sentence was entered.") # If the input is empty, this line prints a message indicating that no sentence was entered.
        return # This line exits the function early if no sentence was entered.
    words = text.split() # This line splits the input text into a list of words using whitespace as the delimiter and stores it in the variable words.
    total_words = len(words) # This line calculates the total number of words in the list   
    longest_word = max(words, key=len) # This line finds the longest word in the list of words using the max function with the key parameter set to len, which returns the length of each word.
...
if __name__ == "__main__": # This line checks if the script is being run directly (not imported as a module).
        sentence_analysis("") # If the script is run directly, this line calls the sentence_analysis function with an empty string as an argument.
